...
class HistoryPagingSource(private val token: String, private val database: CakrawalaDatabase) :  PagingSource<Int, HistoryResponseItem>() {

     companion object {
        const val INITIAL_PAGE_INDEX = 1
        fun snapshot(items: HistoryResponse): PagingData<HistoryResponseItem> {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, HistoryResponseItem> {
        return try {
            ...
            val responseData = database.historyDao().getAllHistory()
